...
| 中 | `MultiModalItemKNN` 支持 `IndexFlatIP`、HNSW 和 IVF | 已覆盖 `ann_topk`/后端参数校验、已见物品过滤和批量/单用户结果一致性；候选截断规模上界和 PQ 仍待补齐 |

当前 FAISS 默认路径是精确的 `IndexFlatIP`，`ann_backend="hnsw"`/`"ivf"` 才是近似 ANN；不同模型的 embedding 不同，也不能单独证明论文实现正确。
...
4. **FAISS 检索**：`[faiss]` extra + `MultiModalItemKNN(use_ann=True)` 默认使用 `IndexFlatIP`，
   也支持 HNSW 和 IVF；缺依赖时自动回退 NumPy 精确内积，PQ 等其他索引仍未实现。
