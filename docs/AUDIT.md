...
服务器现已提供，但本轮文档审核未重新验证 GPU 环境或完成真实数据 benchmark。
...
- generation 目录目前不会自动清理；频繁保存大型历史索引可能累积磁盘占用，需要后续加入保留数/容量策略，并在并发读安全的前提下回收旧 generation。

**（3）多模态与规模：**
- item 特征仍是整体进内存的 dense 矩阵，尚无 mmap、分片编码或完整 ANN 训练路径；
- 图模型的相似度计算已动态限块，但仍可能产生 `chunk x item_count` 临时矩阵；
- 用户特征（`modalities["user"]`）目前只校验，未被任何模型消费，因此新用户冷启动仍有限；
