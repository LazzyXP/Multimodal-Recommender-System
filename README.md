...
- **冷启动**：物品特征可用于内容推荐；用户特征目前只校验和持久化，未进入模型计算。
- **检索**：FAISS 路径使用 `IndexFlatIP` 精确索引，尚未提供 HNSW/IVF 近似检索。
- **模型文件**：仅加载可信来源的 pickle。SHA-256 用于发现意外损坏，不提供来源认证；模型与元数据分别替换，尚不构成目录级事务保存。
...
`benchmarks/` 用于内部测量方法效果、训练耗时与资源占用，不作为独立发布产品。
